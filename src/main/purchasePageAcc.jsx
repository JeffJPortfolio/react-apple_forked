...
function PurchaseACC() {
  const {productDetailNumAcc} = useParams();
  ...
  useEffect(() => {
    // 제품 기본 정보 가져오기
    axios.get(`http://localhost:9000/api/productAcc/${productDetailNumAcc}/info`)
      .then(response => {
    ...
    // 제품 이미지 가져오기
    axios.get(`http://localhost:9000/api/productAcc/${productDetailNumAcc}/productimages`)
      .then(response => {
    ...
    // 추가 이미지 가져오기
    axios.get(`http://localhost:9000/api/productAcc/${productDetailNumAcc}/infoImages`)
      .then(response => {
  ...
  // 관련 악세사리 가져오기
  axios.get(`http://localhost:9000/api/product/relatedAccs`).then(response => {
    ...
    <>
      ...
      <div className="wrap">
        ...
        <section className="yc-acc-model-images-section">
          ...
          <div className="yc-acc-selection-section">
            ...
            <div className="yc-acc-price-section">
              <p className="yc-acc-price">{productAccInfo.productPrice}</p>
              <button 
        ...
        <section className="yc-recommended-accessories">
            ...
            {relatedModelsAcc.length > 0 ? (
                relatedModelsAcc.map((acc, index) => (
                  <div key={index} className="yc-accessory-item">
                    ...
                    <Link to={`/purchaseAcc/${acc.productDetailNum}`} className="yc-add-to-cart-button">장바구니에 담기</Link>
                  </div>
                ))
              ) : (
                <p>추천 액세서리가 없습니다.</p>
          ...
          <Link to="/purchaseAcc" className="yc-more-products-button">더 많은 제품</Link>
        </section>
